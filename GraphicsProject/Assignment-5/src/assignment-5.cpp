...
{
    glm::vec3 initA = -(G[1]) + 3.0f * G[2] - 3.0f * G[3] + G[4];
    glm::vec3 initB = 3.0f * (G[1]) - 6.0f * (G[2]) + 3.0f * (G[3]);
    glm::vec3 initC = (-3.0f) * (G[1]) + 3.0f * (G[2]);
    glm::vec3 initD = G[1];

    float stepSize;
    glm::vec3 temp;
    for (int i = 0; i <= N; i++) {
        stepSize = float(i) / float(N);
        temp = initA * pow(stepSize, 3.0f) + initB * pow(stepSize, 2.0f) + initC * stepSize + initD;
...
void SampleFWD(BezierRow const& G, int N, std::vector<glm::vec3>& Vertices)
{
    ...
    float step = float(1) / float(N);

    // initialize
    glm::vec3 fi = initD;
    glm::vec3 Dfi = initA * pow(step, 3.0f) + initB * pow(step, 2.0f) + initC * step;
    glm::vec3 DDfi = 6.0f * initA * pow(step, 3.0f) + 2.0f * initB * pow(step, 2.0f);
    glm::vec3 DDDfi = 6.0f * initA * pow(step, 3.0f);

    for (int i = 0; i <= N; i++) { 
        // push our points, starting on f0 
        Vertices.push_back(fi);
...
bool Flatness(BezierRow const& G, float epsilon)
{
    ...
    glm::vec3 u = G1G4 / length(G1G4);

    float d1 = length(G1G2 - dot(G1G2, u) * u);
    float d2 = length(G4G3 - dot(G4G3, u) * u);
    ...
    if (dot(G1G2, u) > length(G1G4)) {
        return true;
    }
    if (-dot(G4G3, u) > length(G1G4)) {
        return true;
    }
    if (d1 > epsilon) {
        return true;

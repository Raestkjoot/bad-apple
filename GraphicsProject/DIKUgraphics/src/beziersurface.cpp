...
void BezierSurface::subdivide_bezierpatch(BezierPatch const& G, int level)
{
    if (level == 0) {
        ...
        // Calculate normals
        glm::vec3 N1 = cross(G[2][1] - V1, G[1][2] - V1);
        glm::vec3 N2 = cross(G[4][2] - V2, G[3][1] - V2);
        glm::vec3 N3 = cross(G[3][4] - V3, G[4][3] - V3);
        glm::vec3 N4 = cross(G[1][3] - V4, G[2][4] - V4);
    ...
    // Otherwise: recursive subdivide

    BezierPatch GLL = transpose(this->DBL) * G * this->DBL;
    BezierPatch GRL = transpose(this->DBR) * G * this->DBL;
    BezierPatch GLR = transpose(this->DBL) * G * this->DBR;
    BezierPatch GRR = transpose(this->DBR) * G * this->DBR;

    this->subdivide_bezierpatch(GLL, level - 1);
    this->subdivide_bezierpatch(GRL, level - 1);
    this->subdivide_bezierpatch(GLR, level - 1);
    this->subdivide_bezierpatch(GRR, level - 1);

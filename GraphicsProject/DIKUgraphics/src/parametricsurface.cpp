...
void ParametricSurface::SampleSurface()
{
    ...
    for (float u = this->umin; u < this->umax; u += this->delta_u) {
        for (float v = this->vmin; v < this->vmax; v += this->delta_v) {
            // vertices
            glm::vec3 V1 = Vertex(u, v);
            glm::vec3 V2 = Vertex(u + this->delta_u, v);
            glm::vec3 V3 = Vertex(u + this->delta_u, v + this->delta_v);
            glm::vec3 V4 = Vertex(u, v + this->delta_v);

            // normals
            glm::vec3 N1 = Normal(u, v);
            glm::vec3 N2 = Normal(u + this->delta_u, v);
            glm::vec3 N3 = Normal(u + this->delta_u, v + this->delta_v);
            glm::vec3 N4 = Normal(u, v + this->delta_v);
